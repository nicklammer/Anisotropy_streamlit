...
def logplot(df_input, y_ax, plot_title, filename, plot_dict, outdir, normalized):
    ...
    for i, row in df.iterrows():
        ...
        ax1.plot(
            ...
            y_fit,
            color=row["Color"],
            linewidth=plot_dict["line width"],
            linestyle=row["Line style"],
        )
...
def build_fit_table(df):
    ...
    df_param = pd.DataFrame(
        params_2d_list, columns=column_labels, index=param_labels_list
    )
